...
import {
  buildClassABSchematic, buildClassDSchematic,
  buildDiscreteClassABSchematic, buildDiscreteClassDSchematic,
  buildPseudo3DView
...
function SchematicViewer({
  ...
  const icForSchema = mainIC.split(' ')[0];
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        {view === 'normalized' && <>Schéma normalisé pour <strong style={{ color: 'var(--accent-cyan)' }}>{icForSchema}</strong>{isIntegrated ? ' — MOSFETs internes' : ''}</>}
        {view === 'discrete' && <><strong style={{ color: 'var(--accent-cyan)' }}>Broches en rouge</strong> — B=Base · C=Collecteur · E=Émetteur</>}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      <div className="results-grid">
        {[
          ...
          { icon: <Thermometer size={14} />, label: 'DISSIPATION', value: results.pdTotal, unit: 'W', sub: `Efficacité: ${results.efficiency}%`, color: results.pdTotal > 30 ? 'var(--accent-red)' : 'var(--accent-orange)' },
      ...
      <div className="glass-card" style={{ borderColor: 'rgba(0,242,255,0.2)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
          <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '0.75rem', padding: '1rem' }}>
            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '0.75rem' }}>Paramètres Techniques</div>
          ...
          <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '0.75rem', padding: '1rem' }}>
            <div style={{ fontSize: '0.72rem', color: 'var(--text-muted)', fontWeight: 600, marginBottom: '0.5rem' }}>Observations</div>
            ...
            <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>
              {params.ampClass === 'Class D'
                ? `${mainIC} — rendement ${results.efficiency}% — puissance max ${results.realPower.toFixed(0)}W sur ${params.loadImpedance}Ω.`
                : `${mainIC} — THD minimal — ${params.targetPower}W sur ${params.loadImpedance}Ω.`}
            </p>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
        ...
        <button onClick={onSave} style={{
          ...
          gap: '0.75rem', fontSize: '0.95rem', fontFamily: 'inherit'
        }}>
