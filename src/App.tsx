      ...
      <div className="app-container" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '2rem', padding: '2rem' }}>
        <div className="glass-card animate-fade" style={{ maxWidth: '800px', width: '100%', textAlign: 'center', padding: '3rem 2rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '2rem', marginBottom: '3rem' }}>
            {[
              { Icon: Cpu, label: 'Calculs Automatiques', color: 'var(--accent-cyan)', bg: 'rgba(0,242,255,0.1)' },
              { Icon: ShieldAlert, label: "Détection d'Erreurs", color: 'var(--accent-red)', bg: 'rgba(255,75,43,0.1)' },
              { Icon: FileText, label: 'Rapports PDF', color: '#00ff80', bg: 'rgba(0,255,128,0.1)' },
            ].map(({ Icon, label, color, bg }) => (
              <div key={label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.5rem' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '0.8rem', background: bg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <button className="download-btn" style={{ margin: '0 auto', padding: '1rem 3rem', fontSize: '1.1rem' }} onClick={() => setScreen('analyzer')}>
        ...
        {history.length > 0 && (
          <div className="glass-card animate-fade" style={{ maxWidth: '800px', width: '100%', padding: '2rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              ...
              <button onClick={clearHistory} style={{ background: 'transparent', border: 'none', color: 'var(--accent-red)', cursor: 'pointer', fontSize: '0.85rem', textDecoration: 'underline' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
              {history.map((a) => (
                <div key={a.id}
                  onClick={() => { setParams(prev => ({ ...prev, targetPower: a.power, loadImpedance: a.impedance, supplyVoltage: a.vcc, ampClass: a.architecture.includes('D') ? 'Class D' : 'Class AB' })); setScreen('analyzer'); }}
                  style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', background: 'rgba(255,255,255,0.03)', borderRadius: '0.8rem', border: '1px solid rgba(255,255,255,0.05)', cursor: 'pointer', transition: 'transform 0.2s' }}
                  onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.06)'; e.currentTarget.style.transform = 'translateX(5px)'; }}
                ...
                >
                  <div style={{ display: 'flex', gap: '1.5rem', alignItems: 'center' }}>
                    ...
                    <div style={{ fontWeight: 700 }}>{a.power} W</div>
                    <div style={{ color: 'var(--text-muted)' }}>{a.impedance} Ω</div>
                    <div style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>{a.architecture}</div>
                    <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{a.date}</div>
                  </div>
                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <div style={{ padding: '0.3rem 0.8rem', borderRadius: '1rem', fontSize: '0.75rem', fontWeight: 700, background: a.verdict === 'Functional' ? 'rgba(0,255,128,0.1)' : 'rgba(255,75,43,0.1)', color: a.verdict === 'Functional' ? '#00ff80' : 'var(--accent-red)' }}>
                      {a.verdict}
                    </div>
                    <button onClick={e => { e.stopPropagation(); deleteAnalysis(a.id); }} style={{ background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.2)', cursor: 'pointer', padding: '0.4rem', borderRadius: '0.4rem' }}
      ...
      <header className="header animate-fade" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', textAlign: 'left', marginBottom: '2rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', fontSize: '0.9rem', color: 'var(--text-muted)' }}>
            ...
            Mode Expert
            {!isPremium && <span style={{ fontSize: '0.6rem', padding: '0.1rem 0.4rem', background: 'var(--accent-blue)', color: 'white', borderRadius: '0.4rem', fontWeight: 800 }}>PRO</span>}
          </label>
          <button onClick={() => setScreen('home')} style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-glass)', color: 'white', padding: '0.6rem 1.2rem', borderRadius: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem' }}>
